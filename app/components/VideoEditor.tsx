...
const GOLD = '#D4AF37'; // Un dorado más tenue y premium
...
// ── INTERFACES UNIFICADAS Y ESTRATÉGICAS ───────────────────────────────────
interface Clip {
  numero: number;
  titulo: string;
  ...
  score_viral: number;
  // Campos estratégicos para la Dirección de Arte de Portadas
  portada_prompt_ia: string;
  portada_texto_impacto: string;
}

interface Cliente { id: string; nombre: string; nicho: string; dolor_principal: string }
interface Props {
  theme?: 'dark' | 'light'
  // IMPORTANTE: Esta lista debe venir dynamic del panel de Clientes
  clientes?: Cliente[] 
...
// ── COMPONENTE CLIP CARD PROFESIONAL INTEGRADO ───────────────────────────
function ClipCard({ 
  clip, 
  theme, 
  handlePreviewClip,
...
// ── COMPONENTE COLOR PICKER (Advanced) ────────────────────────────────────
...
// ── COMPONENTE PRINCIPAL (VIDEO EDITOR PANEL) ──────────────────────────────
export default function VideoEditor({ theme = 'dark', clientes = [], onUpload }: Props) {
  ...
  // Estados Maestros del Modal Flotante de CHAR
  ...
  const copyProfessionalData = (text: string) => {
    ...
    alert('✨ Datos estratégicos copiados al portapapeles de CHAR');
  }

  // Resto de estados (Configuración)
  const [sesion, setSesion] = useState('')
  // Vinculación al panel de Clientes
  ...
  const procesar = async () => {
    ...
    try {
      ...
      if (inputTipo === 'archivo') {
        ...
        // IMPORTANTE: Pasamos los datos del cliente seleccionado al backend para el upload
        const r = await onUpload(videoFile, cliente || undefined) 
        urlTemporal = r.url;
      } else {
      ...
      setStep('🎧 CHAR CORE: Transcribiendo y Traduciendo Audio...')
      ...
      setStep(`🧠 CHAR CORE: Ejecutando framework PAS/AIDA y Dirección de Arte...`)
      // IMPORTANTE: Enviamos toda la información estratégica del cliente seleccionado al prompt
      const analysisRes = await fetch('/api/video-editor', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: urlTemporal, transcript, cantidad, tipo, formato, idioma, traducir, idiomaDestino, clientData: cliente }) });
  ...
  return (
    <div style={{ display:'grid', gap:28, maxWidth:1600, margin:'0 auto', padding:'20px', minHeight:'100vh', fontFamily:'inherit' }}>
      
      {/* HEADER DE LA AGENCIA CHAR CORE */}
      ...
      {tab === 'procesar' && (
        <div style={{ display:'grid', gridTemplateColumns:'minmax(400px, 1fr) 2fr', gap:28 }}>
          ...
          {/* COLUMNA IZQUIERDA: CONFIGURACIÓN ESTRATÉGICA */}
          <div style={{ display:'grid', gap:20, alignContent:'start' }}>
            ...
            <div style={card()}>
              ...
              <div style={{ display:'grid', gap:16 }}>
                <div><label style={lbl()}>NOMBRE DE SESIÓN AGENCIA</label><input value={sesion} onChange={e => setSesion(e.target.value)} placeholder="Ej: DG Clean - Estrategia Mayo" style={inp()} /></div>
                
                {/* Vinculación al panel de Clientes */}
                <div>
                  <label style={lbl()}>CLIENTE / NÚCLEO</label>
                  <select value={cliente?.id || ''} onChange={(e) => setCliente(clientes.find(c => c.id === e.target.value) || null)} style={sel()}>
                    <option value="">Seleccionar de Clients Panel...</option>
                    {clientes.map(cl => <option key={cl.id} value={cl.id}>{cl.nombre} ({cl.nich0})</option>)}
                  </select>
                </div>
            ...
            {/* Configuración Visual/Redes */}
            <div style={card()}>
              <div style={lbl()}>DIRECCIÓN DE ARTE & FORMATO</div>
              <div style={{ display:'grid', gap:16 }}>
                <div><label style={lbl()}>FORMATO DESTINO</label><select value={formato} onChange={e => setFormato(e.target.value)} style={sel()}>{FORMATOS.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}</select></div>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', background:c.s2, border:`1px solid ${c.border}`, borderRadius:8, padding:'12px 16px' }}>
                  <div><div style={{ fontSize:13, color:c.text, fontWeight:700 }}>Traducir subtítulos</div><div style={{ fontSize:11, color:c.text3, fontWeight:500 }}>Multi-idioma (Ej: para US/UK)</div></div>
                ...
                {traducir && <div><label style={lbl()}>IDIOMA DESTINO</label><select value={idiomaDestino} onChange={e => setIdiomaDestino(e.target.value)} style={sel()}>{IDIOMAS.map(i => <option key={i}>{i}</option>)}</select></div>}
                <div><label style={lbl()}>COLOR DE SUBTÍTULOS</label><ColorPicker color={colorSub} onChange={setColorSub} /></div>
              </div>
            </div>

            <button onClick={procesar} disabled={loading} style={{ background: loading ? c.border : `linear-gradient(135deg,${GOLD},#8b6010)`, color: loading ? c.text3 : '#05050f', border:'none', borderRadius:12, padding:'18px 24px', fontSize:16, fontWeight:900, cursor: loading ? 'not-allowed' : 'pointer', textTransform:'uppercase' }}>
              {loading ? `⏳ ${step || 'Procesando Nivel Mundial...'}` : `⚡ Auditar y Detectar Patrones Virales`}
            </button>
            {error && <div style={{ background:ERROR+'15', border:`1px solid ${ERROR}40`, borderRadius:12, padding:'16px', color:ERROR, fontSize:14, fontWeight:600 }}>⚠️ ERROR CHAR CORE: {error}</div>}
          </div>

          {/* COLUMNA DERECHA: RESULTADOS ESTRATÉGICOS (sticky video) */}
                ...
                <div style={{ position:'sticky', top:20, zIndex:10 }}>
                  <div style={{background:'#000', border:`1px solid ${c.border}`, borderRadius:14, padding:15, boxShadow:'0 10px 40px rgba(0,0,0,0.3)'}}>
                    ...
                    {urlFinal ? (
                      ...
                      <div style={{ background:'#111', borderRadius:8, padding:'50px 20px', textAlign:'center', color:'#666', fontSize:14, fontWeight:500 }}>Link de YouTube: Usar timestamps locales.</div>
                    )}
                  </div>
                </div>
      ...
      {/* ── MODAL MAESTRO STRATEGIC AUDIT FLOTANTE (CHAR CORE) ───────────────────── */}
          ...
          <div style={{ background: c.surface, border: `1px solid ${GOLD}60`, borderRadius: 20, width: '100%', maxWidth: 1200, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 50px rgba(0,0,0,0.6)', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: 30, display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 30 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                <div>
                  <span style={{ fontSize: 10, color: c.text3, fontWeight: 800, letterSpacing: 1, display: 'block', marginBottom: 6 }}>🎯 GANCHO VIRAL RECOMENDADO</span>
                ...
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    <textarea readOnly value={activeClip.copy_caption_professional} style={{ width: '100%', background: c.s2, border: `1px solid ${c.border}`, borderRadius: 8, color: c.text2, padding: '12px', fontSize: 13, height: 200, fontFamily: 'inherit', resize: 'none' }} />
                ...
                <div>
                  <span style={{ fontSize: 10, color: c.text3, fontWeight: 800, letterSpacing: 1, display: 'block', marginBottom: 6 }}>🧠 JUSTIFICACIÓN DE AUTORIDAD CHAR CORE</span>
              ...
              {/* Lado Derecho: Tiempos, Subtítulos y Checklist de Portada */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20, borderLeft: `1px solid ${c.border}`, paddingLeft: 24 }}>
                ...
                {/* NUEVA SECCIÓN: Receta de Portada Profesional */}
                <div style={{ background: `${GOLD}08`, border: `1px solid ${GOLD}20`, borderRadius: 10, padding: 16 }}>
                  <div style={{ fontSize: 11, color: GOLD, fontWeight: 900, letterSpacing: 1.5, marginBottom: 8 }}>🎨 DIRECCIÓN DE ARTE PARA PORTADA (ADRIÁN/CM)</div>
                  
                  <div>
                    <label style={{ ...lbl(), color: GOLD+'BB' }}>TEXTO SUPERPUESTO (CANVA):</label>
                    <div style={{ fontSize: 18, fontWeight: 900, color: GOLD, textTransform: 'uppercase', background: c.s2, padding: '6px 12px', borderRadius: 6, display: 'inline-block' }}>
                      {activeClip.portada_texto_impacto}
                    </div>
                  </div>

                  <div style={{ marginTop: 12 }}>
                    <label style={{ ...lbl(), color: GOLD+'BB' }}>PROMPT FONDO IA (MIDJOURNEY/DALL-E):</label>
                    <div style={{ position: 'relative' }}>
                ...
                <div style={{ background: c.s2, padding: 16, borderRadius: 10, border: `1px dashed ${c.border}` }}>
                  <span style={{ fontSize: 11, fontWeight: 800, color: c.text, display: 'block', marginBottom: 6 }}>🛠️ CHECKLIST DE REPLICACIÓN (CHAR STANDARDS)</span>
                  <ul style={{ margin: 0, paddingLeft: 14, fontSize: 12, color: c.text2, display: 'flex', flexDirection: 'column', gap: 6, lineHeight: 1.4 }}>
                    <li>Aislá el rostro en primer plano y aplicá máscara de enfoque.</li>
                    <li>Incliná el texto entre 3° y 5° grados (Misma tipografíaAnton/Anton).</li>
                    <li>Usá la paleta de colores corporativos del nicho del cliente.</li>
                  </ul>
                </div>
              </div>
